...
export class PaymentController {
  ...
  static async processPayment(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const apiResponse = await axios.post(
        "https://fakepayment.onrender.com/api/pay",
        {
  ...
  static async listPayments(req: Request, res: Response) {
    try {
      const { servicio, email, estado, fecha_inicio, fecha_fin } = req.query;
      const payments = await PaymentModel.getFilteredPayments({
        service: typeof servicio === 'string' && servicio !== '' ? servicio : undefined,
        email: typeof email === 'string' && email !== '' ? email : undefined,
        status: typeof estado === 'string' && estado !== '' ? estado : undefined,
        startDate: typeof fecha_inicio === 'string' && fecha_inicio !== '' ? fecha_inicio : undefined,
        endDate: typeof fecha_fin === 'string' && fecha_fin !== '' ? fecha_fin : undefined,
      }) || [];
